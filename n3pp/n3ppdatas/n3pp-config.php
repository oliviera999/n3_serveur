...
function getLastReadingDate() {
    ...
    $sql = "SELECT MAX(reading_time) AS last_date FROM n3ppData";
...
function getSensorData($start_date, $end_date) {
    ...
    $sql = "SELECT id, TempAir, Humidite, Luminosite, Humid1, Humid4, Humid2, Humid3, HumidMoy, PontDiv, ArrosageManu, SeuilSec, mail, mailNotif, HeureArrosage, resetMode, etatPompe, tempsArrosage, bootCount, reading_time 
            FROM n3ppData 
...
function exportSensorData($start_date, $end_date) {
    ...
    $sql = "SELECT id, TempAir, Humidite, Luminosite, Humid1, Humid2, Humid3, Humid4, HumidMoy, PontDiv, ArrosageManu, SeuilSec, HeureArrosage, resetMode, etatPompe, reading_time 
            FROM n3ppData 
            WHERE reading_time BETWEEN ? AND ?
            ORDER BY reading_time DESC";
    ...
    if ($result->num_rows > 0) {
        ...
        fputcsv($output, ['ID', 'Température', 'Humidité', 'Luminosité', 'Humid1', 'Humid2', 'Humid3', 'Humid4', 'Humidité Moy.', 'PontDiv', 'Arrosage Manu', 'Seuil Sec', 'Heure Arrosage', 'Reset Mode', 'État Pompe', 'Date']);
...
function getLastReadings($start_date, $end_date) {
    ...
    // Requête SQL de base
    $sql = "SELECT id, sensor, version, TempAir, Humidite, Luminosite, Humid1, Humid2, Humid3, Humid4, HumidMoy, ArrosageManu, SeuilSec, HeureArrosage, resetMode, etatPompe, reading_time 
            FROM n3ppData ";
