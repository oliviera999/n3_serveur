...
### Correctif - GPIO 117 forçage pompe : création fiable et exposition API
- **Résumé** : appel `ensureAquariumPumpForceRowExists` aussi au POST données et avant le GET état outputs ; ajout du GPIO `117` dans la liste renvoyée par `getOutputsState` (sinon la page contrôle / le polling ne voyaient pas l'état) ; défaut `OutputCacheService` pour `117` ; réparation des lignes `gpio=117` avec `name` vide (invisibles dans `findAll`) ; journalisation `error_log` si INSERT/UPDATE échoue (PDO ne lève souvent pas d'exception).
