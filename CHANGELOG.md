...
- **R�sum�** : en mode `php -S`, les routes `aquaponie`, `aquaponie-alt`, `msp1` et `n3pp` utilisent d�sormais un contr�leur local de secours sans d�pendance PDO, ce qui permet de v�rifier le rendu Twig, les assets et le routage m?me sans base disponible.
...
### Correctif - page aquaponie-description servie sans d?pendances BDD en local
- **R?sum?** : la route `aquaponie-description` utilise d?sormais un contr?leur l?ger sans services MySQL, ce qui permet de la tester localement m?me sans base.
...
- **R?sum?** : le front controller d?tecte `PHP_SAPI=cli-server` et n'utilise plus `SCRIPT_NAME` pour calculer le basePath, ce qui r?active les routes Slim en local.
...
- **R?sum?** : ErrorAlertService n'initialise plus sa table en constructeur, ce qui permet de lancer le serveur local et de tester les assets sans connexion MySQL imm?diate.
...
- **R?sum?** : ajout d'un `.htaccess` dans `assets/` pour r??crire les CSS/JS/images vers `public/assets` quand le serveur web sert les fichiers statiques avant Slim.
...
- **R?sum?** : correctif galerie: compatibilit? PHP 7.4/8.0 (readonly, match).

---
## [5.0.20] - 2026-03-06

### Ajout - pages galeries photo (MSP1, N3PP, FFP3) style site actuel, pagination, lightbox
- **R?sum?** : pages galeries photo (MSP1, N3PP, FFP3) style site actuel, pagination, lightbox.

---
## [5.0.19] - 2026-03-06

### Correctif - manifest public, logos/scripts avec base_path, route manifest hors auth
- **R?sum?** : manifest public, logos/scripts avec base_path, route manifest hors auth.

---
## [5.0.18] - 2026-03-06

### Correctif - Base path via GLOBALS (plus de d?pendance ? BasePath.php)
- **R?sum?** : Base path via GLOBALS (plus de d?pendance ? BasePath.php).

---
## [5.0.17] - 2026-03-06

### Correctif - Chargement explicite BasePath si autoload non r?g?n?r?
- **R?sum?** : Chargement explicite BasePath si autoload non r?g?n?r?.

---
## [5.0.16] - 2026-03-06

### Correctif - BasePath pour assets (main.css), templates avec base_path, cache cron/deploy
- **R?sum?** : BasePath pour assets (main.css), templates avec base_path, cache cron/deploy.

---
## [5.0.15] - 2026-03-06

### Correctif - Correctif assets : main.css/noscript.css dans liste CSS, route bg-aquaponie.png, image fond aquaponie
- **R?sum?** : Correctif assets : main.css/noscript.css dans liste CSS, route bg-aquaponie.png, image fond aquaponie.

---
## [5.0.14] - 2026-03-06

### Correctif - injection CsrfService pour N3ppDataController et MspDataController
- **R?sum?** : injection CsrfService pour N3ppDataController et MspDataController.

---
## [5.0.13] - 2026-03-06

### Correctif - correction 404 assets (main.css, jQuery, scripts) et meta PWA mobile-web-app-capable
- **R?sum?** : correction 404 assets (main.css, jQuery, scripts) et meta PWA mobile-web-app-capable.

---
## [5.0.12] - 2026-03-06

### Modifi? - mise a jour publication
- **R?sum?** : mise a jour publication.

---
## [5.0.11] - 2026-03-06

### Modifi? - integration contenu index historique (salle n3, lycee Lyautey, contacts) sur page d'accueil
- **R?sum?** : integration contenu index historique (salle n3, lycee Lyautey, contacts) sur page d'accueil.

---
## [5.0.10] - 2026-03-06

### Modifi? - diagnostic erreurs 500 (error_log) et validation ENV test3/s3
- **R?sum?** : diagnostic erreurs 500 (error_log) et validation ENV test3/s3.
...
- **R?sum?** : test script publish-cycle.
...
- **N3ppOutputController** enrichi : injection N3ppSensorRepository, m?mes donn?es contextuelles.
- **Repositories** : getAllForBoard retourne d?sormais id, name, gpio, state ; ajout getLastBoardRequest.
...
- **Liens legacy** : middleware PHP ajout? pour rediriger en 301 les requ?tes GET `iot.olution.info/ffp3/aquaponie` (et tout `/ffp3/*`) vers les nouvelles pages `/*`. Garantit le fonctionnement m?me sans Apache (.htaccess), par ex. avec nginx. Exclusion de `/ffp3/ffp3gallery/` pour ne pas casser l?API galerie.
...
- **Routes** : `/msp1/msp1datas/msp1-data.php` et `/n3pp/n3ppdatas/n3pp-data.php` acceptent d?sormais GET et POST.
...
- **Pages donn?es** : refonte des templates `msp1_data.twig` et `n3pp_data.twig` sur le mod?le de la page aquaponie (r?f?rence) : m?me structure (wrapper, header, nav, main), bandeau hero (modern-header), cartes de statistiques (stat-card avec variantes temp/humidity/light/water), sections avec titres (section-header), tableau d?historique styl?, footer avec version.
...
- **Contr?leurs** : MspDataController et N3ppDataController passent d?sormais `version` et `environment` aux templates pour une nav et un footer coh?rents avec le reste du site.
...
- **Routes de compatibilit?** : msp1, n3pp et galeries photo g?r?s par des contr?leurs Slim (MspPostDataController, N3ppPostDataController, GalleryUploadController, etc.) ? m?mes URLs et formats que les anciens scripts, compatibilit? totale avec les firmwares msp2_5, n3pp4_2 et ESP32-CAM.
...
- **Race condition** : le polling (RealtimeUpdater) d?marre d?sormais apr?s l'initialisation de ChartUpdater et StatsUpdater (d?lai 800 ms), pour que le premier poll mette bien ? jour graphiques et cartes de stats
...
### Correctif - Tuile Eau potager : m?me largeur que les autres et centr?e sur la 2e ligne
...
- **Vue classique (aquaponie)** : r?gle d?j? pr?sente en 768px pour `.stats-grid .stat-card:last-child:nth-child(odd)` (m?me largeur, centrage)
...
- **Routes** : `/aquaponie` (et variantes test, aquamobile) sert d?sormais la vue paysage (aquaponie_alt.twig) ; `/aquaponie-alt` sert la vue classique (aquaponie.twig)
...
- Inclusion de `realtime-styles.css` sur la page de connexion (`login.twig`) pour appliquer la m?me image de fond et animation de fondu que sur le reste du site
...
- Suppression de la contrainte ? m?me hauteur ? sur grand ?cran (media 901px) : Filtrage et ?tat du syst?me repassent en hauteur naturelle
...
- **Graph Param?tres physiques** : LEDs et Nourriture en scatter (m?me visuel que pompes/chauffage), tuile Potager m?me largeur (r?gle last-child supprim?e)
...
- **Filtrage / ?tat du syst?me** : m?me hauteur sur grand ?cran (media min-width 901px, align-items stretch, flex), boutons 1h?6 mois plus compacts (padding, font-size, border)
- Fichier modifi? : `templates/aquaponie_alt.twig`
...
- **Niveaux d'eau** : badge Aquarium en premier (puis R?serve, Potager) ; sur smartphone toutes les stat-cards m?me taille (override `max-width` / `grid-column` en 1 colonne)
- **Filtrage** : champs Date d?but/fin sur la m?me ligne (grille 2 colonnes)
...
- Titre en `section-header alt-section-title-full` (ic?ne + h3), contenu dans `.alt-chemistry-wrapper` (m?me style que `.alt-chart-column`)
...
- Graphique Param?tres physiques : s?ries **LEDs** et **Nourriture** (gros/petits) affich?es dans le m?me panneau que Humidit? et Luminosit? (axe Y d?cal? ? droite), suppression du 4? panneau et de l'espace vide sous le graphique
- Fichier modifi? : `templates/aquaponie_alt.twig`
...
### Changement - Vue paysage : Humidit? et Luminosit? sur un m?me panneau (2 axes Y)
...
- **Cause** : Le middleware global d'authentification ajout? dans v4.9.54 bloquait tous les endpoints `/api/realtime*` et `/api/outputs*` m?me pour les routes publiques
- **Solution** :
...
- **Correction** : Am?lioration du calcul du `basePath` pour d?tecter correctement le chemin de base selon le point d'entr?e utilis? (public/index.php ou index.php racine)
- **Fichiers modifi?s** : `public/index.php`
...
- **JSON compact** : `GET /api/outputs/state` et `/api/outputs-test/state` retournent d?sormais du JSON compact (sans `JSON_PRETTY_PRINT`) pour rester sous 1024 bytes c?t? ESP32-WROOM (buffer limit?).
- **Fichiers modifi?s** : `src/Controller/OutputController.php`
...
- **Correction** : `OutputCacheService::getOutputsState()` retourne d?sormais des **valeurs par d?faut** pour tous les GPIO attendus par l'ESP32 lorsqu'une ligne est absente en BDD. Les valeurs sont align?es avec le firmware (gpio_mapping.h / GPIODefaults) et les migrations SQL (INIT_GPIO_BASE_ROWS.sql).
...
- **Interface d'administration** : Ajout d'une fonctionnalit? pour vider le cache serveur depuis l'interface web
  - Nouveau contr?leur `CacheController` pour g?rer le vidage des caches Twig et DI Container
...
- **Correction erreur 500** : Ajout de la variable `parameter_gpio_map` manquante dans `OutputController`
  - La variable ?tait utilis?e dans le template `control.twig` mais n'?tait pas pass?e par le contr?leur
...
- **Gestion automatique** : Le script `bin/deploy.sh` d?tecte et sauvegarde automatiquement les modifications locales avant le pull
  ...
  - Le d?ploiement peut continuer sans intervention manuelle m?me en cas de modifications locales
...
- **Coh?rence totale** : Tous les ?l?ments interactifs ont maintenant les m?mes transitions, animations et ?tats
...
- **Coh?rence totale** : Tous les templates utilisent maintenant les m?mes versions et meta tags
...
- **Coh?rence visuelle** : Tous les espacements utilisent maintenant le m?me syst?me
...
- **Syst?me de badges unifi?** : Tous les badges utilisent maintenant les m?mes variables
...
- **Raison** : Simplification de l'interface utilisateur, le filtrage des donn?es se fait d?sormais uniquement via la section "Filtrage des donn?es"
...
- **Fonctionnalit?** : Les nouvelles options utilisent la m?me fonction `setPeriod()` que les options existantes pour une coh?rence parfaite
...
- **Diagnostic d'ex?cution** : Permet de d?tecter si le script s'ex?cute et pourquoi les messages n'apparaissent pas
...
- `CleanDataCommand` envoie d?sormais une alerte eau basse d?taill?e via `NotificationService`.
...
### ?? Temps r?el & feedback
- `realtime-updater.js` affiche d?sormais la dur?e depuis la derni?re lecture avec horodatage, g?re les ?tats ? non disponible ? et met en avant le statut temps r?el par badges color?s.
- `realtime-styles.css` enrichi (pills, animations, styles warning/paused/error) pour diff?rencier les ?tats r?seau.

### ?? API & Contr?le
- Les bascules GPIO (`/api/outputs/toggle`) acceptent d?sormais des requ?tes POST JSON s?curis?es (fin des GET querystring, meilleure compatibilit? caches/proxies).
...
### ?? Refactor contr?leurs (Aquaponie / Dashboard / Output / API temps r?el)
...
### ?? Divers
- Renommage et injection des d?pendances n?cessaires dans le container (ajout de `LogService` / `TemplateRenderer`).
- Nettoyage des imports inutilis?s (`Database` dans certains contr?leurs).
...
- **Coh?rence interface** : Tous les GPIO suivent maintenant la m?me logique : `state = 1` = "Activ?", `state = 0` = "D?sactiv?"
...
- **Impact** : L'API fonctionne maintenant m?me si la colonne `requestTime` n'existe pas
...
### ?? CORRECTION - Migration compl?te vers injection de d?pendances
...
- ? Coh?rence : m?me architecture pour PROD et TEST
...
- Badge "HORS LIGNE" m?me si le rafra?chissement automatique fonctionne
...
- **Principe appliqu?** : "Maximum simplification" - Si ?a marche dans les titres `<h3>`, utiliser exactement la m?me structure
...
- **Impact** :
  ...
  - ? Plus de carr?s blancs m?me avec conflits CSS externes
...
- **Impact** :
  - ? Ic?nes Font Awesome forc?es ? s'afficher m?me avec conflits CSS
...
- **Cause** :
  ...
  - Cas o? `update` lui-m?me pourrait ?tre invalide ou incomplet
