...
- **Migration** : architecture FFP3 (Slim 4) déplacée à la racine de `serveur/`. Un seul front controller (`public/index.php`) pour tout le backend.
- **Routes de compatibilité** : msp1, n3pp et galeries photo gérés par des contrôleurs Slim (MspPostDataController, N3ppPostDataController, GalleryUploadController, etc.) — mêmes URLs et formats que les anciens scripts, compatibilité totale avec les firmwares msp2_5, n3pp4_2 et ESP32-CAM.
- **Sécurité** : requêtes préparées PDO, validation clé API, .gitignore (.env, credentials), env.dist sans secrets réels.
