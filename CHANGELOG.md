...
### Refonte compl?te - Pages de contrôle MSP et N3PP (alignement aquaponie-control)
...
- **N3ppOutputController** enrichi : injection N3ppSensorRepository, m?mes données contextuelles.
...
- **n3pp_control.twig** : refonte identique adaptée aux param?tres n3pp (arrosage, seuils, éco-énergie).
...
- **Liens legacy** : middleware PHP ajouté pour rediriger en 301 les requ?tes GET `iot.olution.info/ffp3/aquaponie` (et tout `/ffp3/*`) vers les nouvelles pages `/*`. Garantit le fonctionnement m?me sans Apache (.htaccess), par ex. avec nginx. Exclusion de `/ffp3/ffp3gallery/` pour ne pas casser l?API galerie.
...
### Refonte compl?te - Pages données MSP et N3PP (alignement aquaponie)
...
- **Panneau état du syst?me** : version firmware, version serveur, nombre de mesures, environnement.
...
- **Pages données** : refonte des templates `msp1_data.twig` et `n3pp_data.twig` sur le mod?le de la page aquaponie (référence) : m?me structure (wrapper, header, nav, main), bandeau hero (modern-header), cartes de statistiques (stat-card avec variantes temp/humidity/light/water), sections avec titres (section-header), tableau d?historique stylé, footer avec version.
- **Données affichées** : MSP ? derni?re mesure (TempAirInt/Ext, HumidAirInt/Ext, LuminositeMoy, TempEau, HumidSol, Pluie) + historique récent avec toutes les colonnes ; N3PP ? derni?re mesure (TempAir, Humidite, Luminosite, HumidMoy, Humid1?4, etatPompe) + historique récent complet.
...
- **Redirections 301** : les requ?tes GET vers `/ffp3/*` (liens externes, signets) redirigent vers `/*`. Les POST (firmware) restent en réécriture interne sans redirection.
...
- **Pages données** : `GET /msp1/msp1datas/msp1-data.php` et `GET /n3pp/n3ppdatas/n3pp-data.php` ? affichage derni?re mesure et historique (MspDataController, N3ppDataController, templates msp1_data.twig, n3pp_data.twig).
- **Pages contrôle** : `GET /msp1/msp1control/`, `GET /n3pp/n3ppcontrol/` ? liste des sorties et boutons Allumer/Éteindre (showControlPage) ; `POST .../msp1-outputs-action.php` et `.../n3pp-outputs-action.php` avec `action=set` pour mise ? jour (setOutput).
...
- **Réécriture /ffp3/** : ajout dans `public/.htaccess` de la r?gle `^ffp3/(.*)$ ? $1` pour que les URLs ffp5cs (`/ffp3/post-data`, etc.) fonctionnent lorsque DocumentRoot = `public/`.
- **Alias legacy LVGL_Widgets** : routes `POST /ffp3datas/post-ffp3-data2.php` et `GET /ffp3control/ffp3-outputs-action2.php` (apr?s réécriture) pour compatibilité firmware LVGL ; chemins ajoutés aux chemins publics (auth).
- **Documentation** : `docs/deployment/QUE_FAIRE_COTE_SERVEUR.md` mis ? jour (rétrocompat /ffp3/). Rapport d?audit : `docs/AUDIT_BUGS_STRUCTURE_SERVEUR.md`.
...
- **Migration** : architecture FFP3 (Slim 4) déplacée ? la racine de `serveur/`. Un seul front controller (`public/index.php`) pour tout le backend.
- **Routes de compatibilité** : msp1, n3pp et galeries photo gérés par des contrôleurs Slim (MspPostDataController, N3ppPostDataController, GalleryUploadController, etc.) ? m?mes URLs et formats que les anciens scripts, compatibilité totale avec les firmwares msp2_5, n3pp4_2 et ESP32-CAM.
- **Sécurité** : requ?tes préparées PDO, validation clé API, .gitignore (.env, credentials), env.dist sans secrets réels.
- **Nettoyage** : suppression des anciens dossiers msp1/, n3pp/, msp1gallery/, n3ppgallery/ (remplacés par les routes Slim).
- **Rétrocompat** : .htaccess racine avec réécriture /ffp3/* ; basePath Slim adapté.
...
### Correctif - Mise ? jour temps réel : graphiques et badges apr?s init
- **Race condition** : le polling (RealtimeUpdater) démarre désormais apr?s l'initialisation de ChartUpdater et StatsUpdater (délai 800 ms), pour que le premier poll mette bien ? jour graphiques et cartes de stats
- **Mobile** : délai porté ? 800 ms pour couvrir la création des graphiques Highcharts ? 600 ms sur mobile (createVersionD)
...
### Correctif - Tuile Eau potager : m?me largeur que les autres et centrée sur la 2e ligne
- **Vue paysage (aquaponie_alt)** : derni?re tuile seule (Potager) avec `width: 100%`, `max-width: calc((100% - 10px) / 2)`, `justify-self: center` et marges auto ; r?gle alignée dans la media query 900px
- **Vue classique (aquaponie)** : r?gle déj? présente en 768px pour `.stats-grid .stat-card:last-child:nth-child(odd)` (m?me largeur, centrage)
...
### Changement - Suivi : courbe eau réserve cachée par défaut, axe températures adapté ? la gamme
...
- **Graphique param?tres physiques** : axe Y températures avec `min: null`, `max: null` pour une échelle adaptée ? la gamme des données (sans forcer 0)
- Fichiers modifiés : `templates/aquaponie.twig`, `templates/aquaponie_alt.twig`
...
- **Contrôleur** : commentaires mis ? jour (show = vue classique ? /aquaponie-alt, showAlt = vue paysage ? /aquaponie)
...
- **Listes** : texte repoussé ? gauche (margin-left supprimé, padding-left 1.25em)
...
- **Priorité haute** : attribut `lang="fr"` sur `<html>` ; footer avec copyright ? Syst?me d'aquaponie FFP3 | ? 2025 olution ?
...
- Survol : les caract?res du lien passent en orange (#FF6300) via `color: #FF6300 !important` pour priorité sur les styles externes
- Fichiers modifiés : `templates/aquaponie.twig`, `templates/aquaponie_alt.twig`
...
- **Graph Param?tres physiques** : LEDs ON, Nourriture gros et Nourriture petits affichés sur l?axe des abscisses (bande 5 % en bas, y=0), panneau Humidité/Luminosité ? 45 %
...
### Changement - Vue paysage : Filtrage et État du syst?me empilés, contenu en colonnes
- Cadres ? Filtrage des données ? et ? État du syst?me ? affichés l'un sous l'autre (une seule colonne)
...
- État du syst?me : health-grid en 4 colonnes (Statut, Derni?re réception, Uptime, Lectures) sur une ligne
...
- Marge sous les boutons ? Afficher les mesures ? et ? Télécharger CSV ? réduite (padding-bottom section filtres 4px desktop / 8px mobile, margin-bottom action-buttons ? 0)
- Fichier modifié : `templates/aquaponie_alt.twig`
...
- `body { background: transparent !important }` pour priorité sur le th?me externe
- `#wrapper > .bg` et `#wrapper.fade-in::before` forcés ? transparent pour laisser voir l?image de fond
...
### Correctif - Vue paysage : Filtrage et État du syst?me en hauteur naturelle
- Suppression de la contrainte ? m?me hauteur ? sur grand écran (media 901px) : Filtrage et État du syst?me repassent en hauteur naturelle
- Évite l?écrasement du bloc Filtrage (plus d?ascenseur illisible) et la superposition du contenu État du syst?me sur la section Analyse chimique
- Fichier modifié : `templates/aquaponie_alt.twig`
...
- **Graph Param?tres physiques** : LEDs et Nourriture en scatter (m?me visuel que pompes/chauffage), tuile Potager m?me largeur (r?gle last-child supprimée)
- **Cadre post featured** : overflow contenu (min-width: 0, max-width: 100%, overflow-x/overflow hidden) pour éviter débordement ? droite
- **Section chimie** : gap avant le titre supprimé (margin/padding/::before ? 0, margin-top titre ? 0)
- **Filtrage / État du syst?me** : m?me hauteur sur grand écran (media min-width 901px, align-items stretch, flex), boutons 1h?6 mois plus compacts (padding, font-size, border)
- Fichier modifié : `templates/aquaponie_alt.twig`
...
- **Niveaux d'eau** : badge Aquarium en premier (puis Réserve, Potager) ; sur smartphone toutes les stat-cards m?me taille (override `max-width` / `grid-column` en 1 colonne)
- **Filtrage** : champs Date début/fin sur la m?me ligne (grille 2 colonnes)
...
### Correctif - Vue paysage : cadres Filtrage et État du syst?me lisibles, contenu contenu
...
### Correctif - Vue paysage : superposition section chimie / cadres Filtrage et État du syst?me
...
### Changement - Vue paysage : Suivi des param?tres chimiques aligné sur Niveaux d'eau / Param?tres physiques
- Titre en `section-header alt-section-title-full` (icône + h3), contenu dans `.alt-chemistry-wrapper` (m?me style que `.alt-chart-column`)
...
### Correctif - Vue paysage : Filtrage des données et État du syst?me
- Alignement hauteur : Filtrage et État du syst?me en hauteur égale (min-height: 0, overflow-y: auto sur .filter-section)
...
- Les deux blocs de graphiques (niveaux d'eau, param?tres physiques) prennent la hauteur de leur colonne de badges ? gauche ; `syncChartHeightsToStats()` appelée apr?s création et au resize
- Fichier modifié : `templates/aquaponie_alt.twig`
...
- Graphique Param?tres physiques : hauteur du panneau Humidité/Luminosité/LEDs passée de 25 % ? 70 % (30 % ? 100 %), suppression du blanc entre le panneau et l'axe des abscisses
- Fichier modifié : `templates/aquaponie_alt.twig`
...
- Graphique Param?tres physiques : séries **LEDs** et **Nourriture** (gros/petits) affichées dans le m?me panneau que Humidité et Luminosité (axe Y décalé ? droite), suppression du 4? panneau et de l'espace vide sous le graphique
- Fichier modifié : `templates/aquaponie_alt.twig`
...
### Changement - Vue paysage : Humidité et Luminosité sur un m?me panneau (2 axes Y)
- Graphique Param?tres physiques : panneaux **Humidité** et **Luminosité** fusionnés en un seul avec axe gauche (%) et axe droit (UA), gain de place verticale
- Fichier modifié : `templates/aquaponie_alt.twig`
...
- Les blocs **Réserve d'eau** et **Cycles de marée** (bilan hydrique) sont affichés sous le graphique des niveaux d'eau, en vis-?-vis sur 2 colonnes de largeur similaire
...
- Nouvelle page publique `/aquaponie-description` : description du syst?me, capteurs, actionneurs, firmware et serveurs (embarqué et distant)
...
- **Probl?me** : Les pages aquaponie publiques affichaient "réseau instable" et les POST/GET du firmware ESP32 ne fonctionnaient plus
- **Cause** : Le middleware global d'authentification ajouté dans v4.9.54 bloquait tous les endpoints `/api/realtime*` et `/api/outputs*` m?me pour les routes publiques
- **Solution** :
...
- **Endpoints restés protégés** :
  ...
  - `/api/outputs*/parameters` - modification de param?tres (nécessite authentification)
...
- **Solution** : Ajout d'un middleware global qui intercepte toutes les requ?tes vers les chemins protégés AVANT le routage
...
- **Note** : L'icône a été changée de `fa-broom` ? `fa-cog` pour ?tre cohérente avec la page de supervision
...
- **Cause** : Le `CacheController` avait sa propre vérification de token `ADMIN_CACHE_TOKEN` qui contournait le syst?me d'authentification normal
...
- **Note** : La route `/control` est correctement définie et accessible apr?s authentification via `/ffp3/control` (avec le basePath)
...
### ?? Sécurité - Syst?me d'authentification pour les pages d'administration
- **Ajout** : Implémentation compl?te d'un syst?me d'authentification pour protéger les pages sensibles
- **Fonctionnalités** :
...
- **Probl?me** : Le lien "n3 iot datas" vers `https://iot.olution.info/ffp3/` affichait toujours l'arborescence Apache malgré les corrections précédentes
- **Solution** :
  - Création d'un fichier `index.php` ? la racine qui inclut directement `public/index.php`
  - Ajout de `DirectoryIndex index.php` dans `.htaccess` pour forcer Apache ? utiliser index.php comme fichier par défaut
...
- **Corrections** :
  ...
  - Suppression de la r?gle problématique `RewriteRule ^ffp3/?$ public/index.php` (déj? gérée par le `.htaccess` dans `public/`)
...
- **Probl?me** : Les changements effectués côté serveur (ESP32, autre client) n?étaient plus reflétés sur la page de contrôle : les switches n?étaient mis ? jour que lorsqu?un ? changement ? était détecté par rapport au cache local.
- **Correction** : ? chaque poll, synchronisation systématique des switches avec la réponse serveur (liste construite ? partir de `lastStates` apr?s mise ? jour depuis l?API). Les param?tres continuaient d??tre mis ? jour via `onStatesReceived(states)`.
...
- **Probl?me** : Sur `/control-test` (env test), la page appelait `/api/outputs/state` (prod) au lieu de `/api/outputs-test/state` ? lecture table prod (ffp3Outputs) au lieu de la table test (ffp3Outputs2), donc valeurs par défaut affichées.
- **Corrections** :
...
- **Vérifié** : `control-auto-save.js` utilisait déj? `outputs-test` en test ; realtime (control.twig) utilisait déj? `realtime-test` en test.
...
- **Probl?me** : Avec `Transfer-Encoding: chunked`, l'ESP32 recevait souvent un body incomplet (timeout / déconnexion) ? "Fetch distant fallback: KO, keys=0".
- **Correction** : En-t?te `Content-Length` explicite sur la réponse JSON pour que l'ESP32 sache combien d'octets lire et évite les timeouts en fin de lecture.
...
- **Probl?me résolu** : Lorsque la table `ffp3Outputs` / `ffp3Outputs2` était vide ou sans lignes pour les GPIO demandés, l'endpoint `GET /api/outputs/state` (et `/api/outputs-test/state`) retournait un objet JSON vide, ce qui faisait afficher des valeurs par défaut sur l'ESP32 et l'interface embarquée.
...
- **Probl?me résolu** : Les graphiques Highcharts ne s'affichaient plus sur smartphone (tablette et laptop OK)
- **Causes identifiées** :
  - Module Boost (WebGL) encore chargé : incompatibilités sur certains mobiles
  - Dimensions des conteneurs ? 0 au moment de la création sur mobile (layout différé)
  - Délai d'initialisation trop court sur mobile
- **Corrections apportées** :
  - Suppression définitive du script `boost.js` (déj? désactivé en config, source de conflits)
  ...
  - Délai initial porté ? 600ms sur mobile (au lieu de 300ms)
...
- **Probl?me résolu** : Les graphiques Highcharts s'affichaient en colonnes au lieu de courbes (areaspline) sur laptop
- **Cause identifiée** : Le module Highcharts Boost (accélération WebGL) causait des probl?mes de compatibilité sur certains GPU/navigateurs
- **Corrections apportées** :
...
- **Note technique** : Le module Boost peut ?tre réactivé pour de tr?s grandes quantités de données,
...
- **Probl?me résolu** : Les graphiques Highcharts ne s'affichaient plus sur smartphone
- **Corrections apportées** :
...
- **Intégration dans l'interface** : Ajout d'un lien discret dans le footer de la page d'accueil vers la page d'administration
  ...
  - Acc?s facile sans nécessiter de connexion SSH ou ligne de commande
- **Fonctionnalités** :
...
- **Documentation** : Création d'un audit complet de la page de contrôle distant
  ...
  - Identification des probl?mes critiques, de sécurité et de performance
...
- **Gestion automatique** : Le script `bin/deploy.sh` détecte et sauvegarde automatiquement les modifications locales avant le pull
  ...
  - Le déploiement peut continuer sans intervention manuelle m?me en cas de modifications locales
  - Les modifications peuvent ?tre récupérées plus tard avec `git stash pop` si nécessaire
...
- **Définition du cycle** : Un cycle complet de marée (montée + descente) est maintenant compté comme 1 cycle au lieu de 2
  ...
  - Un cycle est détecté uniquement lorsqu'il y a eu ? la fois une montée ET une descente significatives (> 2 cm)
  - La fréquence des marées refl?te maintenant le nombre réel de cycles complets par heure
...
- **Filtrage amélioré** : Seuil de variation minimale de 2 cm pour la détection des cycles de marée
  - Modifications dans `TideAnalysisService` et `WaterBalanceService`
  - Les variations inférieures ? 2 cm (en valeur absolue) sont maintenant ignorées lors de la détection des changements de direction
...
- **Nettoyage de l'affichage** : Suppression des 3 tuiles "Fen?tre actuelle", "Mesures" et "Période" en haut de page
  - Ces informations étaient redondantes avec la section "Période analysée" déj? présente plus bas dans la page
...
- **Consolidation des informations de connexion** : Les informations de connexion sont maintenant intégrées dans le bloc "Connexions"
  - Déplacement des cartes de statut (Sync temps réel, Derni?re commande, Lectures aujourd'hui, Uptime) dans le bloc `boards-status`
...
- **Réorganisation de la page de contrôle** : La section "Actions rapides" est maintenant affichée en bas de page
  ...
  - Amélioration de la hiérarchie visuelle : les actions rapides sont accessibles apr?s consultation du contenu principal
...
- **Simplification du layout** : Passage de la page de contrôle de 2 colonnes ? 1 colonne
  - Modification de `.control-layout` : `grid-template-columns: 1fr` (une seule colonne)
  - Sidebar et contenu principal affichés verticalement, l'un apr?s l'autre
...
### ?? Suppression - Syst?me de logs temps réel
- **Suppression du panneau de logs temps réel** : Retrait complet du syst?me de logs temps réel de la page de contrôle
...
- **Ajustement du layout** : Adaptation de la grille de layout de 3 colonnes ? 2 colonnes
  ...
  - Mise ? jour des media queries pour supprimer les références ? `.logs-sidebar`
- **Fonctionnalités conservées** : Toutes les autres fonctionnalités de la page de contrôle restent intactes
  - Le syst?me de synchronisation temps réel (`ControlSync`) continue de fonctionner normalement
  - Les autres composants (contrôles, param?tres, statuts) ne sont pas affectés
...
### ?? Harmonisation compl?te - Animations, transitions et états interactifs
- **Syst?me de transitions unifié** : Création d'un syst?me complet de variables CSS
...
- **Harmonisation compl?te** : Remplacement de toutes les valeurs hardcodées par les variables CSS
...
- **Cohérence totale** : Tous les éléments interactifs ont maintenant les m?mes transitions, animations et états
...
- **Harmonisation Font Awesome** : Mise ? jour de tous les templates vers Font Awesome 6.5.1 avec intégrité
...
- **Cohérence totale** : Tous les templates utilisent maintenant les m?mes versions et meta tags
...
- **Syst?me d'espacement unifié** : Création d'un syst?me complet de variables CSS pour les espacements
...
- **Cohérence visuelle** : Tous les espacements utilisent maintenant le m?me syst?me
...
- **Syst?me de typographie unifié** : Création d'un syst?me cohérent avec variables CSS
  ...
  - Polices : syst?me par défaut et monospace pour les logs
- **Syst?me de badges unifié** : Tous les badges utilisent maintenant les m?mes variables
...
- **Variables CSS consolidées** : Fusion de tous les blocs `:root` en un seul syst?me unifié
...
### ?? Harmonisation - Syst?me de design unifié (couleurs, border-radius, box-shadow)
- **Variables CSS** : Ajout d'un syst?me de variables CSS pour cohérence totale
...
- **Cohérence visuelle** : Interface uniforme avec un syst?me de design cohérent
...
- **Centralisation** : Déplacement de la r?gle `.reading-timestamp { display: none; }` vers le fichier CSS externe
...
- **Maintenabilité** : Code plus lisible et plus facile ? maintenir
- **Sections corrigées** :
...
### ??? Suppression - Désactivation du rangeSelector (zoom) sur les graphiques
- **Interface simplifiée** : Suppression de la zone de sélection d'intervalles (rangeSelector) au-dessus ? gauche des graphiques Highcharts Stock
- **Configuration compl?te** : Désactivation du rangeSelector sur tous les graphiques (niveaux d'eau et param?tres physiques) et dans toutes les configurations responsive
...
- **Fonctionnalité** : Les nouvelles options utilisent la m?me fonction `setPeriod()` que les options existantes pour une cohérence parfaite
...
- **Messages debug précoces** : Ajout de messages immédiatement apr?s `createVersionD()` pour vérifier l'exécution
- **Mécanisme de retry** : Syst?me de tentatives multiples (jusqu'? 10) toutes les 200ms pour s'assurer que les graphiques sont créés
...
- **Messages debug détaillés** : Ajout de vérifications compl?tes apr?s création des graphiques (avec setTimeout)
...
- **Aide au débogage** : Permet de vérifier si le probl?me vient de la configuration ou de données avec intervalles réguliers
...
- **Approche multi-niveaux** : Configuration ? 3 niveaux (global, chart, navigator) pour garantir l'espacement proportionnel au temps réel
...
- **Probl?me identifié** : Highcharts Stock regroupait automatiquement les points de données, masquant l'espacement proportionnel au temps
- **Configuration compl?te** : Combinaison de `ordinal: false` sur l'axe X et `dataGrouping: { enabled: false }` sur les séries pour un affichage temporel fid?le
- **Types de séries** : Configuration appliquée ? areaspline, line, scatter et column pour les deux graphiques
...
- **Meilleure lisibilité** : Les intervalles de temps courts sont visuellement proches, les intervalles longs sont espacés, offrant une représentation plus fid?le de la chronologie des données
...
- **Correction overflow** : Ajout de `overflow-x: hidden` sur `.control-sidebar` pour emp?cher les scrollbars horizontales indésirables
- **Correction cartes boards** : Ajout de `overflow: hidden` sur `.board-card` et `.board-activity` pour emp?cher toute scrollbar indésirable dans les cadres des boards
...
- **Résolution probl?me** : Un seul badge reste maintenant, correctement mis ? jour par le JavaScript pour afficher l'état réel de la synchronisation
...
- **Amélioration initialisation** : Meilleure détection du badge dans le DOM et gestion des cas o? le badge n'est pas encore disponible
- **Ajout classes CSS** : Ajout des styles manquants pour les états `warning` et `paused` du badge
- **Amélioration logs** : Ajout de logs de debug pour faciliter le diagnostic des probl?mes de synchronisation
- **Correction timing** : Le badge se met maintenant ? jour correctement apr?s chaque poll réussi vers l'état "online" (SYNC)
...
- Correctif de l'API temps réel : les GPIO non booléens conservent leur valeur (email, seuils) au lieu d'?tre forcés ? `0`.
- Normalisation des timestamps des boards : stockage en UTC, rendu dans `APP_TIMEZONE` via `DateTimeZone` (`BoardRepository`).
- `LogService::sendAlertEmail()` marqué obsol?te et redirigé vers `NotificationService`; ajout de `NotificationService::sendCustomAlert()`.
...
### ??? Sync badges & indicateurs temps réel
- Ajout du polling `RealtimeUpdater` sur la page de contrôle pour synchroniser les badges ? Sync temps réel ?, ? Derni?re commande ?, ? Lectures aujourd'hui ? et ? Uptime ?.
- Mise en place d'un DOM non destructif (spans annotés `data-*`) afin de préserver les composants lints et éviter la re-création des éléments lors des mises ? jour.
...
### ?? Auto-save formulaires contrôle
...
- Cartes actionneurs homog?nes (icônes, badges sync, descriptions) et param?tres regroupés par rubriques.
- Feedback visuel clarifié (badges synchronisation, animations `value-updated`, harmonisation couleurs).
- Panneau statut temps réel compact (statut, derni?re commande, uptime, lectures).
...
- Harmonisation visuelle des panneaux d'état : nouveaux badges de statut animés, info bulles de derni?re réception, formats enrichis pour les compteurs.

### ?? Temps réel & feedback
- `realtime-updater.js` affiche désormais la durée depuis la derni?re lecture avec horodatage, g?re les états ? non disponible ? et met en avant le statut temps réel par badges colorés.
- `realtime-styles.css` enrichi (pills, animations, styles warning/paused/error) pour différencier les états réseau.

### ?? API & Contrôle
- Les bascules GPIO (`/api/outputs/toggle`) acceptent désormais des requ?tes POST JSON sécurisées (fin des GET querystring, meilleure compatibilité caches/proxies).
...
- Mise ? jour de `docs/README.md` : version courante 4.8.7, date de révision et renvois vers les guides actifs.
...
### ?? Refactor contrôleurs (Aquaponie / Dashboard / Output / API temps réel)
...
- **Fiabilité communication** : Correction des probl?mes de communication entre ESP32 et serveur pour les commandes GPIO
...
- **Cohérence interface** : Tous les GPIO suivent maintenant la m?me logique : `state = 1` = "Activé", `state = 0` = "Désactivé"
...
- **Légendes optimisées** : Tailles de police et symboles adaptés ? chaque breakpoint
...
- **Design moderne et discret** : Refonte compl?te de l'interface des logs temps réel
...
- **Probl?me identifié** : L'ESP32 créait automatiquement des lignes avec `name=NULL` dans la table `ffp3outputs` qui se régénéraient apr?s suppression
- **Cause principale** : `PumpService::setState()` utilisait `INSERT ... ON DUPLICATE KEY UPDATE` avec `name=''` (chaîne vide)
- **Cause secondaire** : `syncStatesFromSensorData()` mettait ? jour tous les GPIO sans vérifier s'ils avaient des noms définis
- **Solution** : 
  ...
  - Les GPIO sans nom ne seront plus créés ni mis ? jour automatiquement
...
- **OutputRepository::syncStatesFromSensorData()** : Ajout de conditions pour éviter la mise ? jour des GPIO sans nom
- **PumpService::setState()** : Remplacement d'INSERT par UPDATE pour éviter la création de lignes vides
- **Logs détaillés** : Messages d'erreur plus précis pour diagnostiquer les probl?mes de synchronisation
...
- **Informations de synchronisation** : Affichage de la derni?re sync ESP32 et des délais de protection
...
- **Control-sync.js** : Amélioration de la détection des changements et mise ? jour des badges de statut
...
- **Affichage correct** : L'état affiché correspond maintenant ? l'état réel de la pompe réserve
...
- **Migration BDD requise** : Nécessite mise ? jour des tables en production
...
### ?? Nettoyage
- **Suppression massive de fichiers obsol?tes** : 54 fichiers supprimés pour simplifier la structure du projet
  - 15 fichiers .md de rapports de corrections terminées
  - 7 fichiers .txt d'instructions temporaires
  - 14 scripts .sh de correction/déploiement obsol?tes
...
- **Documentation simplifiée** : Seuls les fichiers de documentation actifs conservés ? la racine
- **Structure optimisée** : Navigation facilitée, maintenance améliorée

### ?? Détails techniques
- **Fichiers supprimés** : Rapports de corrections v4.4.x ? v4.6.x, scripts de diagnostic temporaires, instructions de déploiement obsol?tes
...
### ?? Modifié
- **Réorganisation de l'interface de contrôle** : Déplacement de la section "État des connexions" apr?s "Actions rapides" dans `control.twig`
- **Amélioration de l'expérience utilisateur** : Les informations de connexion sont maintenant affichées en fin de page pour un acc?s plus facile apr?s les actions principales
...
- **Amélioration de la visibilité** : La série la plus importante (aquarium) est maintenant dessinée en dernier pour ?tre plus visible
...
- **Probl?me résolu** : Erreur 500 sur `/api/outputs-test/board/1/status` causée par la colonne `requestTime` manquante
...
- **Badge d'état** : Ajout d'un badge "ACTIF" (vert) ou "INACTIF" (rouge) ? côté du nom de la GPIO
- **Interface améliorée** : Affichage plus clair de l'état avec icônes et couleurs
- **Mise ? jour temps réel** : Le badge d'état se met ? jour automatiquement toutes les 10 secondes
...
- **JavaScript** : Mise ? jour du code temps réel pour inclure le badge d'état
...
- ? Mise ? jour temps réel fonctionnelle

## [4.6.42] - 2025-01-27

### ? Amélioration - Affichage derni?re GPIO sollicitée

#### Simplification de l'affichage Board 1
- **Focus sur l'essentiel** : Affichage uniquement de la derni?re GPIO modifiée au lieu de toutes les GPIO
- **Information pertinente** : Nom de la GPIO, état (actif/inactif), numéro GPIO et heure de modification
- **Mise ? jour temps réel** : L'affichage se met ? jour automatiquement toutes les 10 secondes
- **Interface épurée** : Une seule GPIO affichée avec toutes les informations nécessaires

#### Détails techniques
- **Repository** : Nouvelle méthode `findLastModifiedGpio()` dans `OutputRepository` pour récupérer la derni?re GPIO modifiée
- **Service** : Nouvelle méthode `getLastModifiedGpio()` dans `OutputService`
- **Contrôleur** : Modification de `OutputController` pour utiliser la derni?re GPIO au lieu de toutes les GPIO
- **Template** : Simplification de `control.twig` avec affichage d'une seule GPIO
- **JavaScript** : Adaptation du code temps réel pour la derni?re GPIO uniquement
...
### ?? Impact
- ? Affichage plus simple et pertinent de la derni?re GPIO sollicitée
- ? Interface épurée sans surcharge d'informations
- ? Mise ? jour temps réel de la derni?re activité GPIO

## [4.6.41] - 2025-01-27

### ?? Corrigé
- **Erreur type OutputController** : Correction de l'erreur "Argument #1 ($board) must be of type string, int given" dans `getBoardGpios()`
- **Conversion type** : Ajout de `(string)` pour convertir l'ID de board en chaîne avant l'appel ? `getBoardGpios()`
...
- **Affichage enrichi** : La derni?re requ?te affiche maintenant le nom et l'état de chaque GPIO
- **Mise ? jour temps réel** : L'affichage se met ? jour automatiquement toutes les 10 secondes
...
- **Mise ? jour automatique** : Rafraîchissement toutes les 10 secondes sans rechargement de page
...
- **Solution simple** : Utilisation de `DATE_SUB()` pour éviter les probl?mes de reconnaissance des noms de timezone par MySQL
- **Impact** : Affichage de l'heure marocaine correcte pour toutes les boards (heure européenne - 1h)

## [4.6.37] - 2025-01-27

### ?? Corrigé
- **Décalage horaire Board 1** : Correction du décalage de +2h sur l'affichage de la derni?re requ?te des boards
...
### ?? Détails techniques
- **Fichier modifié** : `src/Repository/BoardRepository.php` - Correction dans 3 méthodes (`findAll()`, `findActiveForEnvironment()`, `findByName()`)
- **Conversion automatique** : MySQL g?re maintenant automatiquement les changements d'heure été/hiver entre Paris et Casablanca
- **Impact** : Affichage de l'heure correcte pour toutes les boards, respect de l'architecture timezone hybride du projet

## [4.6.36] - 2025-01-27

### ?? Corrigé
- **Erreur de syntaxe JavaScript** : Suppression du code JavaScript restant lié ? la santé du syst?me qui causait une erreur "Unexpected token '}'"
...
### ??? Supprimé
- **Section "État du syst?me"** : Suppression compl?te de la section de santé du syst?me de la page de contrôle
- **Code JavaScript associé** : Nettoyage de tout le code JavaScript lié ? la santé du syst?me
...
- **Erreurs 404 sur les appels API** : Correction des erreurs "HTTP 404" lors de la mise ? jour des données de santé du syst?me
...
### ?? Amélioré
- **Réorganisation des sections de contrôle** : Déplacement des sections "État du syst?me" et "Logs temps réel" sous la section "État de connexion" sur les pages de contrôle
- **Amélioration de l'expérience utilisateur** : Meilleure organisation logique des informations de monitoring

### ?? Détails techniques
- **Fichier modifié** : `templates/control.twig` - Réorganisation de l'ordre des sections
- **Impact** : Interface plus intuitive avec les informations de connexion en premier, suivies des détails syst?me et des logs
...
### ?? Détails techniques
- **Probl?me** : Les groupes de routes PROD et TEST définissaient des routes identiques pour `/assets/js/{filename}`, `/assets/css/{filename}`, `/assets/icons/{filename}` et `/service-worker.js`
...
### ?? Détails techniques
- **Probl?me** : Deux groupes de routes (PROD et TEST) définissaient la m?me route `/manifest.json`
...
### ? Amélioration - Panneau d'état du syst?me sur la page de contrôle

#### Ajout d'informations de synchronisation détaillées
- **Nouveau** : Panneau d'état du syst?me similaire aux pages de suivi sur la page de contrôle
- **Fonctionnalités ajoutées** :
  - Affichage du statut du syst?me (En ligne/Hors ligne/Erreur)
  - Derni?re réception de données avec timestamp
  - Uptime du syst?me sur 30 jours
  - Nombre de lectures aujourd'hui
  - Compteur de mise ? jour en temps réel
  - Contrôles de synchronisation intégrés (Mode Live, Fréquence, Sync manuel)
- **Interface** :
  - Design harmonisé avec les autres pages du syst?me
...
- **Probl?me résolu** : Le Board 1 affichait une heure en avance de 2h par rapport ? l'heure réelle
...
#### Syst?me de logs complet pour le suivi des événements
...
- **Événements loggés** :
  ...
  - Appels API et leurs résultats (succ?s/erreur)
  - Synchronisation temps réel (connexion, changements détectés)
  - Sauvegarde des param?tres
  - Mise ? jour des données de santé du syst?me
- **Interface** :
...
- **Intégration** : Compl?tement intégré avec tous les syst?mes existants
- **Fichiers modifiés** :
...
#### Probl?me des switches incohérents résolu
- **Probl?me résolu** : Les switches de la pompe réserve affichaient des états paradoxaux (switch ON mais état "Désactivé")
...
#### Probl?me des timestamps figés résolu
- **Probl?me résolu** : Les timestamps `requestTime` dans la table `ffp3Outputs2` n'étaient plus mis ? jour lors des requ?tes ESP32
- **Cause** : La méthode `syncStatesFromSensorData()` ne mettait ? jour que les `state` mais pas les `requestTime`
- **Solution** : 
...
#### Probl?me de datation des derni?res requ?tes résolu
- **Probl?me résolu** : Les timestamps "Derni?re requ?te" sur la page control-test affichaient une heure incorrecte
...
#### Probl?me GPIO 101 résolu
- **Probl?me résolu** : Le GPIO 101 (mailNotif) affichait des valeurs `NaN` dans la synchronisation temps réel
...
- **Probl?me résolu** : La fonction `autoSaveParameter` utilisait une route inexistante `/ffp3/post-data.php` (404 Not Found)
...
- **Impact** : Les param?tres peuvent maintenant ?tre sauvegardés correctement via l'API
...
- **Probl?me résolu** : La fonction `autoSaveParameter` était définie dans le scope local de `createOutput` et n'était pas accessible globalement
...
- **Impact** : Les champs de param?tres (seuils, horaires, email) peuvent maintenant ?tre modifiés sans erreur JavaScript
...
#### Interface unifiée
- **Harmonisation compl?te** : L'interface de contrôle utilise maintenant le m?me design que la page aquaponie
- **Cohérence visuelle** : Banner syst?me, section headers, cartes et boutons harmonisés
...
- **Banner syst?me** : Informations syst?me avec gradient et icônes cohérentes
...
- **Auto-sauvegarde** : Param?tres sauvegardés automatiquement avec indicateurs visuels
- **Contrôle GPIO** : Tous les actionneurs (pompes, chauffage, nourrissage, syst?me)
- **Environnements** : Support PROD et TEST avec indicateurs visuels

#### Structure technique
- **Template harmonisé** : `control.twig` compl?tement refactorisé
...
- **Sécurité** : Liste blanche des fichiers autorisés pour éviter l'acc?s non autorisé
...
- ? Interface de contrôle enti?rement opérationnelle
...
#### Correction appliquée
- **Ajout de l'accolade fermante** : Ajouté `}` ? la fin du fichier `src/Repository/OutputRepository.php`
- **Validation** : Aucune erreur de linting détectée
- **Test** : Classe `App\Service\OutputService` maintenant chargée avec succ?s ?
...
- **Conservation de la premi?re** : Gardé la déclaration originale (lignes 31-39)
- **Validation** : Aucune erreur de linting détectée
...
- **Investigation compl?te** : Analyse de tous les commits liés au contrôle
...
#### Hypoth?ses de cause racine identifiées
1. **Cache PHP-DI** (? TR?S PROBABLE) :
   - Cache compilé dans `var/cache/` contient des définitions obsol?tes
...
4. **Permissions** (POSSIBLE) :
   - Probl?mes d'écriture dans `var/cache` ou `var/log`
   - Solution : `chmod 755 var/cache var/log`

#### Outils de diagnostic créés
- **`ANALYSE_REGRESSION_CONTROL_v4.6.15.md`** : Analyse compl?te de la régression
...
- **`public/fix-cache.php`** : Interface web de diagnostic et correction
  ...
  - ?? **? SUPPRIMER** apr?s utilisation pour sécurité

#### Plan d'action recommandé
1. **SSH vers serveur** : `ssh oliviera@toaster`
2. **Nettoyer caches** : `bash fix-server-cache.sh`
3. **Ou via web** : Accéder ? `public/fix-cache.php` et cliquer "Nettoyer les caches"
...
- **Probabilité de résolution** : 90% apr?s nettoyage des caches

### ?? Documentation
- Document d'analyse détaillée créé pour référence future
- Scripts de correction réutilisables pour probl?mes similaires
...
- **Bridge legacy** : Création d'un fichier `post-ffp3-data.php` fonctionnel qui dél?gue au contrôleur moderne
...
### ?? Impact
- **Mode LIVE restauré** : Les API temps réel fonctionnent ? nouveau
...
- **Performance améliorée** : Suppression des instanciations manuelles co?teuses
...
### ?? Bénéfices
- **Réduction de ~42%** des fichiers de documentation ? la racine
...
- **Bouton Enregistrer amélioré** : Design moderne avec "Enregistrer tous les param?tres"
...
- **Interface équilibrée** : Colonnes de m?me hauteur pour un aspect professionnel
...
- **Premi?re colonne** : Gestion de l'eau + Nourrissage (sections liées ? l'aquaponie)
...
- ??? Chauffage & Lumi?re : Contrôle des équipements + param?tres de température
- ?? Email de notification : Contrôle des notifications + configuration email
- ?? Syst?me : Contrôle syst?me + param?tres syst?me
...
- **Suppression de la colonne Actions** devenue vide apr?s regroupement des contrôles
...
- **Regroupement final des contrôles** :
  - Boutons "Forçage réveil" et "Reset ESP" déplacés vers la section Syst?me
...
---
## [4.6.1] - 2025-01-27 ?? AMÉLIORATION UX - Regroupement des contrôles Chauffage & Lumi?re

### ?? Amélioration interface
- **Section Chauffage transformée en "Chauffage & Lumi?re"** avec boutons intégrés
- Boutons du radiateur et de la lumi?re déplacés vers la section Chauffage & Lumi?re
- Interface plus cohérente avec séparation claire entre contrôle des équipements et param?tres de température
...
---
## [4.6.2] - 2025-01-27 ?? RÉVOLUTION UX - Sauvegarde automatique des param?tres

### ?? Fonctionnalité révolutionnaire
- **Sauvegarde automatique** : Les param?tres s'enregistrent instantanément d?s qu'ils sont saisis
...
- **Expérience utilisateur fluide** : Plus besoin de penser ? sauvegarder manuellement

### ?? Indicateurs visuels avancés
- **?? Sauvegarde en cours** : Bordure orange et fond jaune clair
- **? Sauvegarde réussie** : Bordure verte et fond vert clair avec icône de succ?s
...
- **Gestion d'erreurs robuste** : Feedback visuel en cas de probl?me réseau
- **Performance optimisée** : Envoi individuel des param?tres modifiés
...
---
## [4.6.1] - 2025-01-27 ?? HARMONISATION COMPL?TE - Interface cohérente et esthétique
...
- **Banner info syst?me** avec gradient harmonisé
...
### ?? Refonte technique
- **Template compl?tement refait** pour la cohérence
...
### ?? Optimisation technique
- Réorganisation compl?te du code Twig des sections
...
---
## [4.5.46] - 2025-01-27 ?? AMÉLIORATION UX - Harmonisation des sections Chauffage et Syst?me
...
- Interface plus cohérente avec séparation claire entre contrôle des pompes et param?tres de niveau
...
- **Enregistrement automatique** des param?tres du formulaire de contrôle
...
### ?? Améliorations techniques
- Syst?me de debounce pour éviter trop de requ?tes simultanées
...
- Symétrie avec les routes TEST qui utilisent déj? `EnvironmentMiddleware('test')`
...
- ? Cohérence : m?me architecture pour PROD et TEST
...
---
## [4.5.40] - 2025-10-14 ?? CRITIQUE - Mise ? jour SÉLECTIVE des outputs (évite écrasement)

### ?? Correction critique
- **Mise ? jour SÉLECTIVE au lieu de COMPL?TE** : seuls les GPIO présents dans POST sont mis ? jour
- Protection contre l'écrasement des valeurs existantes si non envoyées par ESP32
- Vérification `isset($_POST[...])` pour chaque param?tre avant mise ? jour
- Préserve les valeurs configurées manuellement via l'interface web

### ?? Fichiers modifiés
- **Modifié** : `public/post-data.php` - Ajout vérifications isset() pour mise ? jour conditionnelle

### ?? Impact
- ? Si ESP32 n'envoie pas un param?tre ? valeur existante en BDD préservée
...
- L'email est désormais synchronisé ? chaque POST de l'ESP32

### ?? Fichiers modifiés
- **Modifié** : `public/post-data.php` - Mise ? jour correcte du GPIO 100 (email)
...
### ?? Correction importante
- **Les nouveaux GPIO 111-116 ne sont PAS ajoutés ? la table ffp3Data** (historique)
...
- Les valeurs tempsGros, tempsPetits, tempsRemplissageSec, limFlood, WakeUp, FreqWakeUp sont bien reçues et mises ? jour dans outputs
...
- Séparation claire entre données historiques et param?tres de configuration
...
- **Ajout de 6 nouveaux param?tres GPIO virtuels** (111-116) pour une configuration avancée :
...
- **Mise ? jour automatique compl?te de la table outputs** lors de la réception des données ESP32
...
### ?? Fichiers modifiés
- **Modifié** : `src/Domain/SensorData.php` - Ajout des 6 nouveaux param?tres
- **Modifié** : `src/Repository/SensorRepository.php` - Support des nouvelles colonnes en base
- **Modifié** : `public/post-data.php` - Réception et mise ? jour outputs compl?te
...
- **Intégration des contrôles Mode Live dans la boîte "État du syst?me"** : Les contrôles du mode live (activation, auto-scroll, intervalle, etc.) qui étaient dans une fen?tre flottante en bas de l'écran ont été intégrés directement dans le panneau "État du syst?me"
  - Meilleure organisation de l'interface
  - Tous les contrôles temps réel sont maintenant regroupés au m?me endroit
  - Réduction de l'encombrement visuel
  - Interface plus épurée et professionnelle

### ?? Fichiers modifiés
- **Modifié** : `templates/aquaponie.twig`
  - Déplacement des contrôles live (Mode Live, Auto-scroll, Intervalle, Statistiques, Bouton Rafraîchir) dans le panneau État du syst?me
...
- **Modifié** : `public/assets/css/realtime-styles.css`
  ...
  - Les contrôles s'adaptent maintenant au contexte du panneau syst?me

### ? Résultat
- ? Interface plus claire et mieux organisée
- ? Contrôles temps réel facilement accessibles dans le panneau syst?me
...
### ?? Probl?me résolu
- **Badge LIVE affiché m?me quand le syst?me est hors ligne** : Le badge en haut ? droite affichait "LIVE" (vert) m?me lorsque le syst?me ESP32 était hors ligne, ce qui était trompeur pour l'utilisateur
  - Le badge indiquait uniquement l'état du polling (rafraîchissement automatique)
  - Il ne tenait pas compte du statut réel du syst?me physique (ESP32)
  - Impact : L'utilisateur pouvait croire que le syst?me était actif alors qu'il était hors ligne

### ? Solutions implémentées

#### 1. Badge synchronisé avec le statut syst?me réel
- **`public/assets/js/realtime-updater.js`** : Modification de la méthode `updateSystemStatus()`
  ...
  - La méthode `poll()` ne force plus le badge ? "online" pour laisser `updateSystemStatus()` décider

#### 2. Logique améliorée
- Priorité donnée au statut syst?me réel (health.online) sur l'état du polling
- Badge "HORS LIGNE" m?me si le rafraîchissement automatique fonctionne
...
- ? Plus de confusion entre état du polling et état du syst?me physique
- ? L'utilisateur voit immédiatement si le syst?me ESP32 est opérationnel
...
- **Liens vers index.html** : Les liens `https://iot.olution.info/index.html` utilisés dans la navigation ne fonctionnaient plus
  - Le fichier `index.html` ? la racine du projet n'était pas accessible car situé hors du document root (`public/`)
...
- **`src/Controller/HomeController.php`** : Contrôleur dédié ? la page d'accueil
...
### ?? Corrections critiques
- **Cache en production** : Résolution définitive du probl?me o? les modifications déployées n'étaient pas visibles en production
  - Les caches Twig et DI Container n'étaient jamais vidés apr?s un `git pull` sur le serveur
...
- **`.git/hooks/post-merge`** : Hook Git exécuté automatiquement apr?s chaque `git pull`
  - Appelle automatiquement `bin/clear-cache.php` apr?s chaque fusion
  - Garantit que les caches sont toujours ? jour apr?s un déploiement
...
### ?? Impact
- ? **Résolution définitive** : Les modifications sont maintenant toujours visibles apr?s un déploiement
- ? **Automatisation compl?te** : Plus besoin de penser au cache lors des déploiements
...
2. Incrémenter VERSION et mettre ? jour CHANGELOG.md
...
- **Pied de page Control** : Restauration de l'affichage de la version du firmware ESP32 dans le pied de page de l'interface de contrôle
  ...
  - La version du firmware était déj? affichée dans `aquaponie.twig` et `dashboard.twig` mais manquait dans `control.twig`
...
### ? Améliorations UI
- **Barre mode live** : Positionnement centré en bas de page au lieu d'en bas ? gauche
...
### ?? Corrections critiques
- **Redirections obsol?tes** : Toutes les références au chemin `/ffp3/ffp3datas/*` ont été corrigées vers `/ffp3/*`
  - **`.htaccess`** : Ajout d'une r?gle de redirection générique `^ffp3datas/(.*)$ ? /ffp3/$1 [R=301]`
...
- **Dossier obsol?te** : `ffp3datas/` (vide) déplacé vers `unused/ffp3datas_empty/`

### ?? Impact
- Les utilisateurs utilisant encore les anciennes URLs avec `/ffp3/ffp3datas/*` seront automatiquement redirigés
- Les ESP32 configurés avec les anciens chemins continueront de fonctionner grâce ? la redirection 301
...
- **Principe appliqué** : "Maximum simplification" - Si ça marche dans les titres `<h3>`, utiliser exactement la m?me structure
...
### ?? Correction critique
- **Icônes Font Awesome** : Les icônes dans le bloc "Actions" (pompes, radiateur, lumi?re, etc.) ne s'affichaient pas
  ...
  - Suppression des r?gles de police forcées qui interféraient avec Font Awesome
  - Le CSS simplifié ne définit que le positionnement et le style visuel
  - Font Awesome g?re maintenant naturellement l'affichage des icônes
...
- **Fichier modifié** : `templates/control.twig` (lignes 102-120)
  - Réduction de ~75 lignes de CSS complexe ? ~18 lignes simples

### ?? Note technique
- Les autres icônes de la page (titres, liens) fonctionnaient déj? correctement
...
### ?? Corrections
- **État des connexions** : Correction du formatage de la date "Derni?re requ?te" dans l'interface de contrôle
...
- **Interface de contrôle** : Réduction de la taille des cadres d'actions pour équilibrer les deux colonnes
  - Padding réduit de 1rem ? 0.75rem
  - Icônes réduites de 52px ? 44px
  - Taille de police des labels réduite de 1rem ? 0.9rem
  - Taille de police des statuts réduite de 0.85rem ? 0.75rem
  - Switch toggle réduit de 58×32px ? 52×28px
  - Gap entre boutons réduit de 1rem ? 0.75rem
  - Border-radius ajusté de 16px ? 12px
- **Objectif** : Améliorer l'équilibre visuel entre la colonne Actions et la colonne Param?tres
- **Fichier modifié** : `templates/control.twig`
...
#### Probl?me résolu : Carrés blancs ? la place des icônes Font Awesome
...
- **Solution appliquée** : 
  - ? Ajout r?gles CSS ultra-spécifiques pour `::before` et `::after`
...
- **Solution de repli SVG** (ligne 16) :
  ...
  - Fonctionne m?me si police WOFF2 bloquée
...
- **Impact** :
  ...
  - ? Plus de carrés blancs m?me avec conflits CSS externes
...
---
## [4.5.23] - 2025-10-13 ?? Chauffage et Syst?me côte ? côte

### ? Améliorations

#### Affichage des cadres Chauffage et Syst?me sur la m?me ligne
- **Amélioration** : Les cadres "Chauffage" et "Syst?me" sont maintenant côte ? côte
...
#### Organisation compacte des param?tres de contrôle
- **Amélioration** : Réorganisation de tous les param?tres pour un affichage plus compact
- **Nourrissage - Horaires** : Matin, Midi, Soir sur la m?me ligne (3 colonnes)
- **Nourrissage - Durées** : Gros poissons, Petits poissons sur la m?me ligne (2 colonnes)
...
- **Impact** :
  ...
  - ? Lisibilité améliorée avec regroupement logique des param?tres
...
#### Affichage des durées de nourrissage sur la m?me ligne
- **Amélioration** : Les champs "Gros poissons" et "Petits poissons" sont maintenant côte ? côte
...
- **Impact** :
  ...
  - ? Meilleure lisibilité des param?tres de nourrissage
...
- **Probl?me** : Les icônes Font Awesome n'apparaissent toujours pas sur certains navigateurs (cases blanches)
...
- **Solutions appliquées** :
  ...
  - Renforcement des r?gles CSS avec plus de sélecteurs spécifiques
...
- **Impact** :
  - ? Icônes Font Awesome forcées ? s'afficher m?me avec conflits CSS
  - ? Détection automatique des probl?mes de chargement
...
#### Correction du refill pompe réservoir qui se rép?te sans s'arr?ter
- **Probl?me identifié** : 
  ...
  - La pompe démarre/arr?te de façon répétée sans respecter la durée configurée
...
- **Cause racine** :
  ...
  - **Côté ESP32** : S'attend ? une logique normale (`pump_tank=1` = ON, `pump_tank=0` = OFF)
...
- **Scénario du bug** :
  ...
  6. ? la prochaine synchro ? retour ? l'étape 3 (boucle infinie)
  
- **Solution appliquée** :
  - Inversion de la logique dans `OutputController::getOutputsState()` pour GPIO 18
  - GPIO 18 = 0 (hardware) ? `pump_tank=1` (envoyé ? l'ESP32)
  - GPIO 18 = 1 (hardware) ? `pump_tank=0` (envoyé ? l'ESP32)
...
- **Impact** :
  - ? Élimine le cycle infini de démarrage/arr?t
...
- **Tests ? effectuer** :
  ...
  - [ ] Vérifier que la pompe s'arr?te apr?s la durée configurée
  - [ ] Vérifier que `pump_tank` refl?te bien l'état réel de la pompe
  - [ ] Vérifier les autres GPIO (pompe aquarium, lumi?re, chauffage)
...
- **Probl?me identifié** : 
  ...
  - Se produit lors de la mise ? jour des graphiques en temps réel
...
- **Cause** :
  - Le tableau `series.data` de Highcharts peut contenir des entrées `null` ou `undefined` apr?s certaines opérations
...
- **Impact** :
  ...
  - ? Améliore la robustesse des mises ? jour en temps réel
...
#### Correction du probl?me de lignes dupliquées dans ffp3Outputs
- **Probl?me identifié** : 
  ...
  - Cause : Le `PumpService.php` créait une nouvelle ligne ? chaque `UPDATE` infructueux, sans vérifier l'existence de doublons
...
- **Solutions appliquées** :
  ...
  **1. Modification du PumpService.php**
  ...
  - **Apr?s** :
    ...
    ```
  - Cette syntaxe MySQL/MariaDB évite les doublons et met ? jour la ligne existante automatiquement
  ...
  **2. Création des scripts de migration SQL**
  - `migrations/FIX_DUPLICATE_GPIO_ROWS.sql` :
    ...
    - Vérifications avant/apr?s pour validation
  ...
  - `migrations/README.md` : Documentation compl?te de la procédure d'application des migrations

- **Impact** :
  - ? Plus aucune création automatique de doublons grâce ? la contrainte UNIQUE
...
- La syntaxe `ON DUPLICATE KEY UPDATE` est spécifique ? MySQL/MariaDB
- Les deux environnements (PROD et TEST) sont traités par les scripts de migration
- Le probl?me n'affectait que l'environnement PROD car TEST avait probablement moins d'exécutions CRON
...
- **Probl?me** : Erreur `TypeError: Cannot read properties of undefined (reading 'x')` ? la ligne 225
  - Se produisait lors de la mise ? jour temps réel des graphiques
...
- **Impact** : Les graphiques se mettent désormais ? jour en temps réel sans erreur
...
- La vérification `p &&` garantit que l'objet existe avant d'accéder ? ses propriétés
...
### ?? Notes
- Les liens dans `index.html` étaient déj? corrects
...
- **Migration compl?te de `ExportController` vers PSR-7**
  ...
  - **Apr?s** : Objets PSR-7 `Request` et `Response` correctement utilisés
  ...
  - Prévention des probl?mes potentiels de buffer mixing
...
- ? `HeartbeatController` (déj? PSR-7)
- ? `RealtimeApiController` (déj? PSR-7)
- ? `OutputController` (déj? PSR-7)

Contrôleurs HTML (moins critiques) :
- ?? `AquaponieController` (legacy - ? migrer ultérieurement)
- ?? `DashboardController` (legacy - ? migrer ultérieurement)
- ?? `TideStatsController` (legacy - ? migrer ultérieurement)
...
- **Correction du probl?me HTTP 500 sur `/post-data-test` et `/post-data`**
  ...
  - **Symptômes** : Messages de réponse concaténés ("Données enregistrées avec succ?s" + message d'erreur)
...
### ?? Contexte technique
Cette correction résout le probl?me identifié lors de l'analyse des logs ESP32 o? :
...
- **Correction du probl?me "GPIO NaN changed" dans les logs de la console**
  - L'API `/api/outputs/state` retourne ? la fois des clés numériques (GPIOs) et des clés textuelles (noms comme "mail", "heat", "light") pour la compatibilité ESP32
...
- **Correction du décalage de +1h au chargement initial de la page aquaponie**
  ...
  - Solution : Appel immédiat de `statsUpdater.updateSummaryDates()` apr?s initialisation
  - Les dates sont maintenant cohérentes d?s le chargement initial avec le timezone Africa/Casablanca
...
- **Correction de l'affichage "NaN" dans le champ email**
  ...
  - Implémentation d'une logique de typage intelligent :
    ...
    - GPIO 100 (email) : conservation comme chaîne de caract?res
    - Autres param?tres : tentative de conversion en nombre, sinon conservation comme chaîne
...
---
## [4.7.0] - 2025-10-13 ?? Gestion timezone et fen?tre glissante améliorées

### ? Nouvelles fonctionnalités

#### Fen?tre glissante en mode live
- **Implémentation d'une fen?tre d'analyse glissante** (6h par défaut)
  - Au chargement : Affiche la période demandée (historique)
  - En mode live : La fen?tre glisse automatiquement pour maintenir la durée fixe
...
- **Indicateur visuel du mode d'analyse** avec badge animé
  ...
  - Badge `LIVE` (rouge pulsant) : Fen?tre glissante active avec données temps réel
...
- **Plus de probl?mes de décalage** avec utilisateurs dans différents fuseaux
...
- `public/assets/js/stats-updater.js`
  - Propriétés pour fen?tre glissante (`slidingWindow`, `windowDuration`)
  - Séparation compteurs (`initialReadingCount`, `liveReadingCount`)
  - Méthode `updatePeriodInfo()` avec logique fen?tre glissante
...
- `docs/TIMEZONE_MANAGEMENT.md`
  ...
  - Gestion fen?tre glissante expliquée
  - Tableau récapitulatif mis ? jour

### ?? Corrections de bugs

- **Fix : Période d'analyse s'étendant indéfiniment** en mode live (remplacé par fen?tre glissante)
...
- **Fix : Durée calculée incorrectement** en mode live (fen?tre glissante fixe)

### ?? Impact utilisateur

- ? **Affichage en heure locale réelle** (Casablanca) pour les utilisateurs au Maroc
- ? **Fen?tre d'analyse stable** qui ne s'étend plus indéfiniment
...
- **Probl?me** : Les icônes Font Awesome n'apparaissaient pas dans l'interface de contrôle
- **Causes identifiées** :
...
- **Refonte compl?te du design des boutons de contrôle** (pompes, lumi?res, etc.)
...
- **Les dates affichaient 10:00 au lieu de 09:00 (heure réelle Casablanca)**
  - Timestamps BDD stockés en heure de Paris (+1h par rapport ? Casablanca)
  - Configuration serveur PHP : `APP_TIMEZONE=Europe/Paris`
  - Affichage doit ?tre en `Africa/Casablanca` pour montrer l'heure locale réelle
...
- **BDD** : Timestamps stockés en heure de Paris (car serveur ? Paris)
...
- **Passage de Europe/Paris ? Africa/Casablanca pour l'affichage**
  - Le projet physique (aquaponie, ESP32) est situé ? **Casablanca**
...
### ?? Important - Différence avec le serveur
- **Serveur web** : Hébergé ? Paris (`Europe/Paris`)
...
### ?? Impact utilisateur
- ? Les dates affichées correspondent ? l'heure locale du projet ? Casablanca
...
### ?? Note pour l'avenir
Si nécessaire de revenir ? l'heure de Paris (serveur), il suffit de changer :
...
- Initialement configuré sur `Europe/Paris` mais devait ?tre `Africa/Casablanca`
...
### ? Ajouté
- **Mise ? jour en temps réel de TOUTES les informations temporelles**
  ...
  - Toutes les périodes affichées (titre + banni?re) synchronisées
...
- **Mise ? jour de TOUTES les statistiques des cartes**
  ...
  - 7 capteurs × 4 stats = 28 valeurs mises ? jour en temps réel

### ?? Amélioré
- **Module stats-updater.js considérablement étendu**
  - Nouvelle méthode `updateStatDetails()` : Met ? jour min/max/avg/stddev
  - Nouvelle méthode `updatePeriodInfo()` : G?re les timestamps de période
  - Nouvelle méthode `updateSummaryDates()` : Met ? jour toutes les dates affichées
...
- **Template aquaponie.twig avec IDs ajoutés partout**
  ...
  - Total : 38 nouveaux IDs ajoutés pour permettre les mises ? jour
...
- ? Badge LIVE et état syst?me

**TOTAL : 42 éléments** mis ? jour automatiquement toutes les 15 secondes !
...
- **Erreur JavaScript : "Identifier 'realtimeUpdater' has already been declared"**
  ...
  - Utilisation de `window.realtimeUpdater` pour accéder ? la variable globale
...
- **Documentation du plan de correction mode live**
  ...
  - Documentation détaillée des probl?mes identifiés
  - Plan d'implémentation complet avec exemples de code
  - Guide de tests détaillé pour validation

### ?? Maintenance
- Incrémentation de version suite ? la documentation du plan
...
- **Mismatch des IDs dans stats-updater.js**
  ...
  - Les cartes de niveaux d'eau se mettent maintenant ? jour correctement en temps réel
...
- **Cartes de statistiques pour param?tres physiques dans aquaponie.twig**
  ...
  - Section dédiée "Param?tres physiques" avec icônes appropriées
  - Toutes les cartes s'animent lors des mises ? jour en temps réel

- **Module control-values-updater.js pour la page de contrôle**
  - Mise ? jour automatique de l'état des connexions boards
  - Synchronisation des valeurs des param?tres affichés dans les formulaires
...
- **Mise ? jour en temps réel étendue ? la page de contrôle**
  - Les états des boards se mettent ? jour automatiquement
  - Les switches se synchronisent (déj? implémenté v4.5.0)
  - Les param?tres affichés se mettent ? jour
...
### ?? Fichiers créés
- `public/assets/js/control-values-updater.js` : Module de mise ? jour pour page de contrôle (189 lignes)

### ?? Impact utilisateur
Les utilisateurs peuvent maintenant :
- ? Voir TOUTES les valeurs (eau + températures + humidité + luminosité) se mettre ? jour en temps réel
- ? Observer les changements avec des animations visuelles claires
- ? Avoir des informations compl?tes sur chaque param?tre (valeur actuelle + min/max/moyenne/écart-type)
- ? Utiliser le mode live sur la page d'aquaponie ET sur la page de contrôle
- ? Bénéficier de la mise ? jour automatique en environnements PROD et TEST
...
### ? Nouveau Design
- **Boutons d'action enti?rement redessinés** dans la page de contrôle
...
- **Responsive amélioré** : Adaptation optimale sur tous les formats d'écran
  ...
  - Tr?s petits écrans : Optimisation spéciale (< 400px)
- **Feedback visuel instantané** lors du changement d'état
  - Mise ? jour immédiate du texte de statut
...
- Mise ? jour JavaScript pour feedback visuel immédiat

### ?? Fichiers modifiés
- `templates/control.twig` : Refonte compl?te du HTML et CSS des boutons d'action
- JavaScript `updateOutput()` : Ajout de mise ? jour visuelle instantanée
...
---
## [4.5.0] - 2025-10-12 ?? Mode Live - Mise ? jour temps réel des graphiques

### ? Ajouté
- **Mode live avec mise ? jour automatique des graphiques en temps réel**
  - Les graphiques Highcharts se mettent ? jour automatiquement sans rafraîchir la page
  - Mise ? jour dynamique des cartes de statistiques (niveaux d'eau, températures, humidité, luminosité)
...
- **Panneau de contrôle du mode live**
  ...
  - Toggle auto-scroll des graphiques pour suivre les derni?res données
  - Sélecteur d'intervalle de mise ? jour (5s, 10s, 15s, 30s, 60s)
  - Compteur des nouvelles données reçues
  - Bouton "Rafraîchir maintenant" pour forcer une mise ? jour immédiate
...
- **`realtime-updater.js` étendu**
  ...
  - Optimisation : récup?re uniquement les nouvelles données depuis le dernier timestamp
  - Gestion intelligente du premier poll (derni?re lecture) vs polls suivants (lectures incrémentielles)
...
- **Performances optimisées**
  ...
  - Désactivation automatique des animations si > 100 points ? ajouter
...
- ? Observer les cartes de statistiques se mettre ? jour en temps réel
- ? Activer/désactiver le mode live selon leurs besoins
- ? Configurer l'intervalle de mise ? jour (5s ? 60s)
- ? Voir les graphiques suivre automatiquement les derni?res données (auto-scroll)
...
- Highcharts Boost déj? chargé pour supporter les grandes séries de données
...
---
## [4.4.7] - 2025-10-12 ?? Amélioration UX - Période par défaut

### ?? Amélioré
- **Période d'analyse par défaut réduite ? 6 heures**
  - `AquaponieController` : Période par défaut changée de `-1 day` ? `-6 hours`
  - Graphiques Highcharts : Sélection par défaut changée de "1 semaine" ? "6 heures"
...
- **Ajout `API_SIG_SECRET` dans `.env`**
  ...
  - Permet la sécurisation compl?te de l'API ESP32 avec signature
...
### ?? Documentation
- ? `.gitignore` déj? présent avec `/var/cache/` (validation effectuée)
...
- **Probl?mes critiques** : 2 ? 0 (tous corrigés ?)
- **Probl?mes majeurs** : 3 ? 0 (tous corrigés ?)
- **Probl?mes mineurs** : Réduits de 5 ? 2
...
- **Navigation**: Correction de tous les liens de navigation dans les templates
  ...
  - Résout le probl?me des "liens morts" lors de la navigation
...
- **Asset Routing**: Utilisation de liens symboliques pour l'acc?s aux assets
  ...
  - Solution simple et propre sans r?gles de réécriture complexes
  - Script `DEPLOY_NOW.sh` mis ? jour pour créer automatiquement les liens
  - Garde la structure standard du projet (fichiers publics dans `public/`)

### ?? Contexte
Suite aux erreurs 404 persistantes malgré la correction des chemins en v4.4.2, utilisation de liens symboliques (approche standard et simple) plutôt que de r?gles de réécriture Apache complexes.
...
### ?? Contexte
Le serveur web pointe déj? vers le dossier `public/` comme document root, donc les URLs ne doivent pas inclure `/public/` dans le chemin.
...
- **Interface unifiée** : Toutes les pages (aquaponie, dashboard, tide-stats, control) ont maintenant :
  - Le m?me niveau de modernité
  - Le m?me syst?me temps réel
  - Le m?me support PWA
...
- Absence de syst?me temps réel sur dashboard et tide-stats
...
### ?? Corrigé
- **Probl?me d'affichage sur smartphone** : Les boutons et actionneurs ne dépassent plus de leur container sur petits écrans
...
- **Switches** : Réduction de la taille des interrupteurs sur mobile (scale 0.7) et tr?s petits écrans (scale 0.6 pour <400px)
...
- **Lisibilité** : Tailles de police adaptatives sur tr?s petits écrans (<400px)
...
### ?? Impact
- Meilleure visibilité sur la gestion de l'eau du syst?me aquaponique
...
### ?? Modifié
- **Graphiques des param?tres physiques** : Ajout d'un effet d'ombrage (area fill) pour les courbes de température (eau et air), humidité et luminosité
  - Type de graphique changé de `line` ? `areaspline` pour les séries concernées
  - Ajout de dégradés colorés sous les courbes avec `fillColor` (opacité de 0.3 ? 0.05)
  - Configuration `fillOpacity: 0.3` ajoutée dans les `plotOptions` pour cohérence
  - Harmonisation visuelle avec les graphiques des niveaux d'eau qui avaient déj? cet effet
...
### ? Ajouté
- **Synchronisation temps réel pour l'interface de contrôle** : L'interface `/control` se met maintenant ? jour automatiquement pour refléter les changements côté serveur
  ...
  - **Badge LIVE** en haut ? droite indiquant l'état de la synchronisation :
    ...
    - ?? **ERREUR** : Échec apr?s plusieurs tentatives
...
- Fonction `updateOutput()` modifiée pour forcer une synchronisation immédiate apr?s un toggle manuel (délai 500ms)

### ?? Documentation
- Cette fonctionnalité était prévue dans `TODO_AMELIORATIONS_CONTROL.md` et `IMPLEMENTATION_REALTIME_PWA.md`
- Permet une expérience collaborative : plusieurs utilisateurs peuvent contrôler le syst?me simultanément
...
- Pas de surcharge serveur : requ?tes lég?res (JSON simple avec paires GPIO/state)
...
### ?? Modifié
- Mise ? jour du pied de page pour inclure la version du firmware ESP32 ? côté de la version de l'application web
...
- **Composer update requis** : Exécuter `composer update` apr?s pull
...
- **Dashboard Syst?me Temps Réel** : Panneau d'état du syst?me
  - Statut online/offline avec indicateur visuel
  - Derni?re réception ESP32 (format "il y a X min/h")
...
- **Service Worker** : `service-worker.js`
  ...
  - Mode offline avec derni?res données en cache
...
- **Template aquaponie.twig** :
  ...
  - Panneau "État du syst?me" avec 4 métriques temps réel
...
- **Nouvelles variables .env** :
  ...
  - `PUSH_VAPID_PUBLIC_KEY` : Clé publique VAPID (? générer)
  - `PUSH_VAPID_PRIVATE_KEY` : Clé privée VAPID (? générer)
...
- **Dashboard syst?me** : Métriques en temps réel
...
#### ?? Performance
- **Polling optimisé** : Requ?tes lég?res (JSON)
- **Cache intelligent** : Service worker avec fallback
- **Lazy loading** : Scripts chargés apr?s DOM ready
...
- [ ] Mise ? jour temps réel des graphiques Highcharts
...
1. `git pull` pour récupérer les derni?res modifications
...
### ?? Notes importantes
- **Polling** : Gén?re 4 requ?tes/minute par utilisateur actif
...
#### Corrigé
- **Bug critique CleanDataCommand** : `checkWaterLevels()` utilisait `min()` au lieu de la derni?re lecture réelle
...
- **RestartPumpCommand** : Nouvelle commande pour gérer le redémarrage différé des pompes
  ...
  - Utilise un syst?me de flag file pour programmer les redémarrages
  - Permet au CRON de ne plus ?tre bloqué pendant 5 minutes
...
- **Injection de dépendances (DI)** : Implémentation de PHP-DI v7
  ...
  - Les contrôleurs ne peuvent plus ?tre instanciés manuellement sans le container
...
#### ?? Refactorisé
- **AquaponieController** : Réduit de 301 ? ~180 lignes (-40%)
...
- Testabilité grandement améliorée grâce ? l'injection de dépendances
...
- **Nettoyage lignes vides excessives** : Suppression des lignes vides inutiles dans tous les fichiers PHP
  - `src/Config/Database.php` : Réduit de 87 ? 44 lignes
  - `src/Service/SensorStatisticsService.php` : Réduit de 260 ? 128 lignes
...
- **Script de nettoyage** : `tools/cleanup_whitespace.php`
  ...
  - R?gles : max 1 ligne vide entre méthodes, aucune dans les blocs de code
...
- **`LogService`** : Ajout @deprecated sur `sendAlertEmail()` (? déplacer dans NotificationService)
...
- **Page d'accueil moderne** : Nouvelle page `index.html` avec présentation des projets IoT
  ...
  - Banni?re informative sur le projet pédagogique olution
  ...
  - Style cohérent avec les autres pages (m?me charte graphique)
...
- **Gestion de session** : Transfert des données POST lors des redirections pour compatibilité compl?te
...
- **Liens externes préservés** : Pas besoin de mettre ? jour les liens externes
...
- **Banni?re période** : Affichage moderne avec gradient et informations centralisées
...
- **Icônes spécifiques** : Fl?ches, vagues, graphiques pour chaque type de donnée

### ?? UX/UI
- **Cohérence visuelle** : M?me charte graphique sur toutes les pages (aquaponie, dashboard, tide-stats, control)
...
### ?? Responsive
- **Grilles adaptatives** : Stats-grid et quick-filters s'adaptent ? l'écran
...
- **Param?tres physiques** : Aires colorées sur 3 axes Y + scatter pour LEDs et nourriture
...
- **Version D définie comme version par défaut** : S'affiche automatiquement au chargement de la page
  - Bouton Version D en premi?re position avec icône étoile ?
  ...
  - Optimisation : Lazy loading conservé pour A, B et C (chargement ? la demande)
...
- **Version D - Stock + Aires colorées (Mix B+C)** : Le meilleur des deux mondes !
  ...
  - **Légendes compl?tes** : Toutes les séries affichées dans la légende
...
- Aires colorées pour chaque param?tre avec dégradés
...
- **Erreur Highcharts Stock corrigée** : `Highcharts.stockChart is not a function`
  ...
  - Highstock.js inclut déj? toutes les fonctionnalités de Highcharts standard
...
- **Version B (Stock Navigator) corrigée** : Fonctionnement maintenant opérationnel
  ...
  - Ajout d'un 4?me axe Y pour les états des équipements (LEDs, nourriture)
...
- **Graphique Param?tres physiques** : 4 axes Y (températures 25%, humidité 25%, luminosité 25%, états 10%)
- **Navigator** : Hauteur fixée ? 40px pour meilleure visibilité
...
- **3 versions de graphiques Highcharts** inspirées des visualisations météo pour améliorer la lisibilité
  - **Version A - Graphiques empilés synchronisés** : 4 graphiques séparés (niveaux, températures, humidité/lumi?re, équipements) avec zoom synchronisé style météo
...
- **Interface graphiques** : Chargement lazy des versions B et C (créées uniquement ? la premi?re sélection)
...
- **Responsive** : Tous les graphiques s'adaptent ? la taille d'écran
