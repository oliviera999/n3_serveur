...
**Nourrissage manuel (page contrôle FFP3, v6.0.0+ — compteur monotone)** : `POST /api/outputs*/trigger-feed` (auth session, CSRF) — corps JSON `{ "id": <output_id>, "gpio": 108|109 }` (plus de `step`). L'appel fait `state = state + 1` uniquement si la ligne `id` correspond bien au GPIO demandé ; réponse `{ success, gpio, counter, feed_cmd_id }`. Le `state` de 108/109 est un **entier croissant** (= nombre total de repas demandés), jamais remis à zéro par le serveur. L'ESP32 lit ce compteur via GET `outputs/state`, le compare à son propre compteur exécuté (NVS) et rattrape l'écart (un repas par poll, plafonné à 5). Plus de séquence reset/trigger, plus de front 0→1, plus d'acquittement firmware. Un client legacy envoyant `step:"reset"` est traité en no-op. Voir section nourrissage dans `SYNCHRONISATION_BIDIRECTIONNELLE.md`.
