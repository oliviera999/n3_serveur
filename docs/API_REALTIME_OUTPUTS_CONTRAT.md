...
**Les trois domaines** exposent aussi `GET alerts/active` — liste des alertes actives (placeholder, retourne `[]`). Route enregistrée pour FFP3, MSP1 et N3PP (handler `getActiveAlerts`).
...
#### GET `alerts/active` (FFP3, MSP1, N3PP)
...
| MSP1 | `GET/POST /msp1/api/outputs/toggle` | `gpio` **ou** `name`, `state` (0\|1), `board` (opt.) | `gpio` prioritaire si fourni, sinon `name` |
| N3PP | `GET/POST /n3pp/api/outputs/toggle` | `gpio`, `state` (0\|1), `board` (opt.) | Par GPIO |
...
Pour l’aquaponie, cinq environnements existent : prod, test, test3, s3, s3test. Les préfixes sont :
...
| s3test | `/api/realtime-s3-test` | `/api/outputs-s3-test` |
