...
class OutputCacheService
{
    ...
    private function maybeAttachAndConsumeOtaTrigger(PDO $pdo, string $env, array &$result): void
    {
        try {
            ...
            if ($this->isMissingTableException($e)) {
                try {
                    $this->createOtaTriggerTableIfMissing($pdo);
                } catch (PDOException $createError) {
                    error_log('[OutputCacheService] creation ffp3OtaTrigger impossible: ' . $createError->getMessage());
                }
    ...
    private function createOtaTriggerTableIfMissing(PDO $pdo): void
    {
    ...
    public function setTriggerOtaCheckRequested(): void
    {
        $env = TableConfig::getEnvironment();
        try {
            $this->upsertOtaTrigger($this->pdo, $env);
        } catch (PDOException $e) {
            if ($this->isMissingTableException($e)) {
                $this->createOtaTriggerTableIfMissing($this->pdo);
                $this->upsertOtaTrigger($this->pdo, $env);
    ...
    private function upsertOtaTrigger(PDO $pdo, string $env): void
    {
