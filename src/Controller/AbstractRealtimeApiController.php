...
abstract class AbstractRealtimeApiController
{
    ...
    public function getOutputsState(Request $request, Response $response): Response
    {
        try {
            ...
            // reflashable garde sa config locale). On n'acquitte PLUS les one-shots
            // dans ce cas. Avant, l'ack tournait quand même alors que la réponse
            // restait nested-only → GPIO 110 (reset) / 13 (arrosage manuel n3pp)
            // étaient consommés sans jamais être vus par le firmware déployé
            // (URL `/api/outputs/state` + X-Api-Key). Les commandes restent en
            // attente jusqu'au passage en `safe`/`full`, ou jusqu'à un GET sur la
