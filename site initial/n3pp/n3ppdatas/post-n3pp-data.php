...
$api_key = $sensor = $version = $TempAir = $Humidite = $Luminosite = $Humid1 = $Humid2 = $Humid3 = $Humid4 = $HumidMoy = $PontDiv = $WakeUp = $ArrosageManu = $SeuilSec = $FreqWakeUp = $SeuilPontDiv = $mail = $mailNotif = $HeureArrosage = $resetMode = $etatPompe = $tempsArrosage = $bootCount = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($api_key == $api_key_value) {
        ...
        $sql = "INSERT INTO n3ppData (sensor, version, TempAir, Humidite, Luminosite, Humid1, Humid2, Humid3, Humid4, HumidMoy, PontDiv, ArrosageManu, SeuilSec, FreqWakeUp, SeuilPontDiv, mail, mailNotif, HeureArrosage, resetMode, etatPompe, tempsArrosage, bootCount)
        VALUES ('" . $sensor . "', '" . $version . "', '" . $TempAir . "', '" . $Humidite . "', '" . $Luminosite . "', '" . $Humid1 . "', '" . $Humid2 . "', '" . $Humid3 . "', '" . $Humid4 . "',  '" . $HumidMoy . "',  '" . $PontDiv . "', '" . $ArrosageManu . "', '" . $SeuilSec . "', '" . $FreqWakeUp . "', '" . $SeuilPontDiv . "', '" . $mail . "', '" . $mailNotif . "', '" . $HeureArrosage . "', '" . $resetMode . "', '" . $etatPompe . "', '" . $tempsArrosage . "', '" . $bootCount . "');
        UPDATE msp1Outputs SET state = '" . $ArrosageManu . "' WHERE gpio= '109';
        ...
        UPDATE msp1Outputs SET state = '" . $HeureArrosage . "' WHERE gpio= '104';
        UPDATE msp1Outputs SET state = '" . $tempsArrosage . "' WHERE gpio= '105';
