...
-->
<?php
    include_once('/home4/oliviera/iot.olution.info/n3pp/n3ppcontrol/n3pp-database.php');
    ...
    if ($result3) {
        while ($row = $result3->fetch_assoc()) {
            if ($row["gpio"] == "100"){
                ...
                $HeureArrosage = $row["state"];
            }
            else if ($row["gpio"] == "105"){
                $tempsArrosage = $row["state"];
            }
...
<html>
	<head>
        ...
        <link rel="stylesheet" href="https://iot.olution.info/n3pp/n3ppcontrol/n3pp-style.css" />
				 ...
				 <!-- Nav -->
					<nav id="nav">
						<ul class="links">
							...
							<li class="active"><a href="https://iot.olution.info/n3pp/n3ppdatas/n3pp-data.php"></a>phasmopolis</li>
					...
					<div id="main">
						 <!-- Featured Post -->
							<article class="post featured">
								<header class="major">
                                    ...
                                    <div>
                                        <form onsubmit="return createOutput();">
                                            ...
                                            <label for="outputGpio">Heure arrosage</label>
                                            <input type="number" name="HeureArrosage" min="0" id="HeureArrosage" value=<?php echo $HeureArrosage; ?>>
                                            <label for="tempsArrosage">Temps arrosage (en ms)</label>
                                            <input type="number" name="tempsArrosage" min="0" id="tempsArrosage" value=<?php echo $tempsArrosage; ?>>
                                ...
                                <div>
                                    <center>                         
                                        <a href="https://iot.olution.info/n3pp/n3ppdatas/n3pp-data.php" class="button large">Retour aux données</a>
                                    </center>
                                </div>
                            </article>
                        </div>
    ...
    <script>
        function updateOutput(element) {
            ...
            if(element.checked){
                xhr.open("GET", "https://iot.olution.info/n3pp/n3ppcontrol/n3pp-outputs-action.php?action=output_update&id="+element.id+"&state=1", true);
            }
            else {
                xhr.open("GET", "https://iot.olution.info/n3pp/n3ppcontrol/n3pp-outputs-action.php?action=output_update&id="+element.id+"&state=0", true);
            }
            xhr.send();
        }
        ...
        function deleteOutput(element) {
            ...
            if (result) {
                var xhr = new XMLHttpRequest();
                xhr.open("GET", "https://iot.olution.info/n3pp/n3ppcontrol/n3pp-outputs-action.php?action=output_delete&id="+element.id, true);
                xhr.send();
        ...
        function createOutput(element) {
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "https://iot.olution.info/n3pp/n3ppcontrol/n3pp-outputs-action.php", true);
            ...
            var HeureArrosage = document.getElementById("HeureArrosage").value;
            var tempsArrosage = document.getElementById("tempsArrosage").value;
            var WakeUp = document.getElementById("WakeUp").value;
            var FreqWakeUp = document.getElementById("FreqWakeUp").value;
            var httpRequestData = "action=output_create&mail="+mail+"&mailNotif="+mailNotif+"&SeuilSec="+SeuilSec+"&SeuilPontDiv="+SeuilPontDiv+"&HeureArrosage="+HeureArrosage+"&tempsArrosage="+tempsArrosage+"&WakeUp="+WakeUp+"&FreqWakeUp="+FreqWakeUp;
